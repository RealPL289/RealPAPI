...
public class RealPAPI extends JavaPlugin {
    ...
    public void onEnable() {
        saveDefaultConfig();
        setupLuckPerms();
        new RealPAPIExpansion(this).register();
        Objects.requireNonNull(getCommand("rpapi")).setExecutor(this);
        Objects.requireNonNull(getCommand("rpapi")).setTabCompleter(new RPAPITabCompleter());
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length == 1 && args[0].equalsIgnoreCase("reload")) {
            if (!sender.hasPermission("rpapi.reload")) {
        ...
        sender.sendMessage(colorize("&cИспользование: /rpapi reload"));
        return true;
    }
    ...
    class RealPAPIExpansion extends PlaceholderExpansion {
        private final RealPAPI plugin;
        private final Map<String, String> chatMappings = new ConcurrentHashMap<>();

        RealPAPIExpansion(RealPAPI plugin) {
            this.plugin = plugin;
            reloadMappings();
        }
        ...
        private String handleCustomPlaceholder(Player player, String identifier) {
            ...
            if (value.isEmpty() || value.equals(placeholder)) {
                String configValue = plugin.getConfig().getString("placeholder_messages." + identifier);
                return plugin.colorize(configValue != null ? configValue : "");
            }
            return plugin.colorize(value);
        }
    }

    class RPAPITabCompleter implements TabCompleter {
        @Override
